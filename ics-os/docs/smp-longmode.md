...
`NET_DHCP_OK`, `NET_DHCP_RENEW_OK`, `NET_PING_OK`, `NET_UDP_OK`, `NET_TCP_OK`,
`NET_SOCK_OK`, `NET_SOCK_UDP_OK`, `NET_SOCK_LISTEN_OK`, and `NET_HTTPD_OK`
(SLIRP DHCP DORA+renew; host echo on UDP `:7777`/TCP `:7778`; userland
`netecho.exe` and `httpd.exe` on `:7779`/`:8080` via QEMU `hostfwd`). Softnet
state is serialized by a recursive `net_lock` (with a separate pbuf spinlock);
