...
`NET_DHCP_OK`, `NET_PING_OK`, `NET_UDP_OK`, `NET_TCP_OK`, `NET_SOCK_OK`,
`NET_SOCK_UDP_OK`, and `NET_SOCK_LISTEN_OK` (SLIRP DHCP DORA then host echo
on UDP `:7777` and TCP `:7778`; userland `netecho.exe` TCP client, UDP
`sendto`/`recvfrom`, and TCP listen on `:7779` via QEMU `hostfwd`). Softnet
