...
| Networking | `test-net`, `test-net-unit` | virtio-net MSI-X attach, static SLIRP config, ARP + ICMP echo to `10.0.2.2`, UDP/TCP echo clients to host `:7777`/`:7778` (guest also serves port 7), Berkeley sockets (`FD_SOCK`, syscalls `0xC6`–`0xCE`, `netecho.exe` TCP client + UDP sendto/recvfrom + listen/accept via hostfwd `:17779`→`:7779`); host TAP for checksum/ARP/ICMP/UDP/TCP/sockaddr | No DHCP; single interface; minimal TCP (no window scaling/retransmit storms) |
