...
| Networking | `test-net`, `test-net-stress`, `test-net-unit` | virtio-net MSI-X, RFC 2131 DHCP DORA + unicast renew (`NET_DHCP_OK`/`NET_DHCP_RENEW_OK`) with static fallback, ARP/ICMP/UDP/TCP echo, Berkeley sockets + `httpd.exe` HTTP/1.0 on `:8080` via hostfwd, softnet `net_lock` + `netstress` under `user-smp`; host TAP for checksum/ARP/ICMP/UDP/TCP/sockaddr/DHCP | Single interface; minimal TCP; rebind path implemented but not a separate QEMU gate |
