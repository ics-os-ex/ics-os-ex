...
| Networking | `test-net`, `test-net-stress`, `test-net-unit` | virtio-net MSI-X, RFC 2131 DHCP DORA + T1/T2 timer FSM renew/rebind (`NET_DHCP_OK`/`NET_DHCP_RENEW_OK`/`NET_DHCP_REBIND_OK`) with static fallback, ARP/ICMP/UDP/TCP echo, Berkeley sockets + `httpd.exe` + `nc.exe`, softnet `net_lock` + `netstress` under `user-smp`; host TAP for checksum/ARP/ICMP/UDP/TCP/sockaddr/DHCP | Single interface; minimal TCP; lease timer service is polled from selftest (not a background softnet thread yet) |
