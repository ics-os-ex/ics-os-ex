...
| Networking | `test-net`, `test-net-stress`, `test-net-unit` | virtio-net MSI-X attach, static SLIRP config, ARP + ICMP echo to `10.0.2.2`, UDP/TCP echo clients to host `:7777`/`:7778` (guest also serves port 7), Berkeley sockets (`FD_SOCK`, syscalls `0xC6`–`0xCE`, `netecho.exe` TCP/UDP/listen), recursive softnet `net_lock` + pbuf spinlock, concurrent `netstress.exe` under `user-smp` (`NETSTRESS_PASS`); host TAP for checksum/ARP/ICMP/UDP/TCP/sockaddr | No DHCP; single interface; minimal TCP (no window scaling/retransmit storms) |
