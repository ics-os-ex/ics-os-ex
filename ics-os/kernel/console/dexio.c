...
static int serial_line_start = 1;

static int serial_write_uint_nb(unsigned long v) {
  char buf[16];
  int i = 0;
  if (v == 0) {
    buf[i++] = '0';
...
static int serial_write_timestamp_nb() {
  unsigned long t = (unsigned long)getprecisetime();
  serial_putchar('[');
  serial_write_uint_nb(t);
  serial_putchar(']');
  serial_putchar(' ');
  return 1;
}
...
void outchar(char x) { putcEX(x); };

void println(char *s, int attbx) { printf("%s\n", s); };
...
int do_printf(const char *fmt, va_list args, fnptr_t fn, void **ptr) {
  ...
  for (; *fmt; fmt++) {
    ...
    case 0:
      ...
      /* found %, get next char and advance state to check if next char is a
       * flag */
      state++;
      fmt++;
      /* FALL THROUGH */
    ...
    case 4:
      ...
      *where = '\0';
      ...
      case 'c':
        ...
        flags &= ~PR_LZ;
        where--;
        *where = (unsigned char)va_arg(args, unsigned);
        actual_wd = 1;
        goto EMIT2;
      case 's':
...
void nextln() { Dex32NextLn(Dex32GetProcessDevice()); };
