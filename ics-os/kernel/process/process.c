...
void mutex_unlock()
{
    ...
    {
        mov eax, 0
        xchg eax, lock_var  // This could have been a single assignment lock_var
= 0 mov value, eax // But we are merely using xchg again to see the previous
value
    }
}
...
DWORD sigpriority = 0; // set this to the process ID of the process that
                       // requires immediate attention
DWORD sigterm =
    0; // set this to the process ID of the process you wish to terminate
...
DWORD sched_sysmes[3] = {
    0, 0, 0}; // scheduler system messages [0] = pid, [1] = mes, [2] = data
...
DWORD getparentid() { return current_process->owner; };

// returns the process id of the current process
DWORD getprocessid() { return current_process->processid; };

// returns the process id of the parent process
DWORD getpprocessid() { return current_process->owner; }

/*creates a USER thread (In DEX this is process which shares the same memory
 * space as its parent, BUGGY process, but has its own stack pointer)
 *
...
DWORD ps_enqueue(PCB386 *process) {
  ...
  bridges_link((devmgr_generic *)cursched, &cursched->ps_enqueue, process, 0, 0,
               0, 0, 0);
};
...
DWORD ps_dequeue(PCB386 *process) {
  devmgr_scheduler_extension *cursched =
      extension_table[CURRENT_SCHEDULER].iface;
  bridges_link((devmgr_generic *)cursched, &cursched->ps_dequeue, process, 0, 0,
               0, 0, 0);
};
...
DWORD forkprocess(PCB386 *parent) {
  ...
  memcpy(pcb, parent, sizeof(PCB386)); // Initialize the new process by copying
                                       // the parent process' PCB
  strcat(
      pcb->name,
      ".fork"); // Add a 'fork' suffix to indicate that it was created by fork
  ...
  pcb->owner =
      parent->processid; // Set the parent to the process id of the parent

  /*Allocate a new page directory*/
  pagedir =
      (DWORD *)mempop(); // obtain a physical address from the memory manager
  pg = (DWORD *)getvirtaddress(
      (DWORD)pagedir);   // convert physical address to a virtual address
  ...
  maplineartophysical((DWORD *)pagedir, (DWORD)SYS_PAGEDIR_VIR,
                      (DWORD)pagedir /*,stackbase*/, 1);
  ...
  maplineartophysical((DWORD *)pagedir, (DWORD)SYS_KERPDIR_VIR,
                      (DWORD)pagedir1 /*,stackbase*/, 1);
...
DWORD createprocess(void *ptr, char *name, DWORD *pagedir, process_mem *pmem,
                    void *stack, DWORD stacksize, DWORD syscallsize,
                    void *dex32_signal, char *params, char *workdir,
                    PCB386 *parent) {
  int pages;
  DWORD flags, *pg;

  PCB386 *temp =
      (PCB386 *)malloc(sizeof(PCB386)); // allocate the PCB for the process
  ...
  temp->accesslevel =
      ACCESS_USER;      // Indicates that the process is a USER process
  ...
  temp->regs.ES =
      USER_DATA; // set the segment registers to memory area for user processes
  temp->regs.SS = USER_DATA;
  temp->regs.CS = USER_CODE;
  temp->regs.DS = USER_DATA;
  temp->regs.FS = USER_DATA;
  ...
  pg = (DWORD *)getvirtaddress(
      (DWORD)pagedir); /*convert to a virtual address so that
                        we could use it here without disabling
                        the paging mechanism of the 386*/

  maplineartophysical2((DWORD *)pg, (DWORD)SYS_PAGEDIR_VIR,
                       (DWORD)pagedir /*,stackbase*/, 1);
  ...
  maplineartophysical2((DWORD *)pg, (DWORD)SYS_KERPDIR_VIR,
                       (DWORD)pagedir1 /*,stackbase*/, 1);
...
DWORD dex32_asyncproc(saveregs *r, void *entrypoint, char *name,
                      DWORD stacksize) {
  PCB386 *temp = (PCB386 *)malloc(sizeof(PCB386));
...
void ps_seterror(int error) { current_process->lasterror = error; };

int ps_geterror() { return current_process->lasterror; };
...
DWORD createkthread(void *ptr, char *name, DWORD stacksize) {
  ...
  PCB386 *temp = (PCB386 *)malloc(sizeof(PCB386)); // allocate PCB for the
                                                   // thread
  DWORD cpuflags;
  ...
  temp->accesslevel =
      ACCESS_SYS; // Set the access level to kernel. This is a kernel thread.
  temp->owner =
      getprocessid(); // The owner of this thread is the current process
  temp->status |= PS_ATTB_THREAD; // Indicate that this PCB is for a thread
  temp->knext =
      knext; // Set the the top of the heap of this thread to same as kernel
  temp->pagedirloc =
      pagedir1; // Set the pagedir for this thread to the first page directory
  temp->workdir = current_process->workdir; // Set the working directory of this
                                            // thread to same as owner

  // set up the initial values of the CPU registers for this thread
  memset(temp, 0, sizeof(saveregs)); // This works because "regs" is the first
  ...
  temp->regs.ES = SYS_DATA_SEL;     // Set the segment registers to appropriate
                                // selectors for kernel memory area.
  temp->regs.SS =
      SYS_STACK_SEL; // this basically tells us that this is a kernel thread
  ...
  temp->arrivaltime =
      getprecisetime(); // Store the arrival/creation time of this thread
  temp->stdin = current_process->stdin; // set the stdin

  /*Try to enter the critical section */
  sync_entercrit(&processmgr_busy); // Access to the process list must synched
                                    // that's why its in the critical section
  dex32_stopints(&cpuflags);
...
/*puts the current process to sleep for a desired amount of time
  in milliseconds*/
void sleep(DWORD val) { current_process->waiting = val; };
...
DWORD kill_process(DWORD processid) {
  ...
  ptr = bridges_ps_findprocess(processid); // obtain a handle to the PCB given
                                           // id
  ...
  if (ptr != -1) { // PCB exists
    ...
    if (!(ptr->status & PS_ATTB_UNLOADABLE)) {
      ...
      while (closeallfiles(ptr->processid) ==
             1) // close all opened files by the process
        ;

      // locate the parent process and decrement its waiting
      // status...important for the dex32_wait() function
      parent = ps_findprocess(
          ptr->owner); // parent should no longer wait for this process
      if (parent != -1) {
...
DWORD kill_thread(PCB386 *ptr) {
  ...
  kill_children(
      ptr->processid); // kill the children of this thread first!!cascade kill
...
int get_processlist(PCB386 **buf) {
  ...
  if (cursched->ps_listprocess == 0) {
    printf("ERROR: schduler extension module does not support "
           "ps_listprocess()..\n");
    return 0;
  };
  ...
  total = bridges_call((devmgr_generic *)cursched, &cursched->ps_listprocess, 0,
                       0, 0);
  *buf = (PCB386 *)malloc(sizeof(PCB386) * total);

  // make an intermodule call to the scheduler
  bridges_call((devmgr_generic *)cursched, &cursched->ps_listprocess, *buf,
               sizeof(PCB386), total);
...
int dex32_wait() {
  ...
  while (current_process->childwait >= waitval &&
         current_process->childwait != 0)
    ;
};
...
DWORD dex32_killkthread_name(char *processname) {
  ...
  for (i = 0; i < total; i++) {
    // if the processname matches the one in the list and the process can be
    // killed, the sigterm global variable is set to the process id to inform
    // taskswitcher() that a process will be terminated
    if (strcmp(ptr[i].name, processname) == 0 &&
        !(ptr[i].status & PS_ATTB_UNLOADABLE)) {
      sigterm = ptr[i].processid; // inform the taskswitcher that a process will
                                  // be terminated
      free(ptr);
      return 1;
    };
  ...
  for (i = 0; i < total; i++) {
    if ((ptr[i].processid == processid) &&
        !(ptr[i].status & PS_ATTB_UNLOADABLE)) {
      sigterm = ptr[i].processid; // inform taskswitcher() that a process is to
                                  // be terminated
      free(ptr);
      return 1;
    };
...
// The taskswitcher() is basically the program that runs all the time, aka CPU
// scheduler. It selects the process to execute on the CPU. The selection of the
  ...
  do {
    ...
    do {
      if (!sigwait) {
        ...
        devmgr_scheduler_extension *cursched =
            (devmgr_scheduler_extension *)extension_table[CURRENT_SCHEDULER]
                .iface;
    ...
    // A non-zero value in sigterm indicates the process id of the process to be
    // terminated. sigterm is usually set in the
    // dex32_killkthread_name(),ps_user_kill(), and exit() functions. do clean
    // up for terminate process a request to terminate a process is received,
    // pid of process to end is the value of sigterm
    if (sigterm && !flushing) {
      flushok = 0;
  ...
  for (i = 0; i < total; i++) {
    if (pid == ptr[i].processid) {
      ...
      printf("EAX=0x%s EBX=0x%s ECX=0x%s EDX=0x%s\n",
             itoa(ptr[i].regs.EAX, temp, 16), itoa(ptr[i].regs.EBX, temp1, 16),
             itoa(ptr[i].regs.ECX, temp2, 16),
             itoa(ptr[i].regs.EDX, temp3, 16));

      printf("EDI=0x%s ESI=0x%s ESP=0x%s Flags=0x%s\n",
             itoa(ptr[i].regs.EDI, temp, 16), itoa(ptr[i].regs.ESI, temp1, 16),
             itoa(ptr[i].regs.ESP, temp2, 16),
             itoa(ptr[i].regs.EFLAGS, temp3, 16));

      printf("Waiting: %d\n", ptr[i].waiting);
      printf("Last system calls:(1) : 0x%s ,(2-last): 0x%s\n",
             itoa(ptr[i].cursyscall[0], temp2, 16),
             itoa(ptr[i].cursyscall[1], temp, 16));
    };
  };
};
...
void show_process() {
  ...
  /*Tell the scheduler to give us an array of PCBs which contain the PCBs of the
  ...
  for (i = 0; i < total; i++) {
    ...
    percent_cpu_time =
        (ptr[i].totalcputime - ptr[i].lastcputime) * 100 / grandtotalcputime;
    ...
    printf(" %-11s %-14s %5dK %5ds %5ds(%2d)%%\n", levelstr, temp, psize,
           ptr[i].arrivaltime / 100, ptr[i].totalcputime / 100,
           percent_cpu_time);
  };
...
void process_init() {
  ...
  kernel->status =
      PS_ATTB_LOCKED |
      PS_ATTB_UNLOADABLE; // it cannot be locked and unloaded, its the kernel!
  kernel->knext = knext;  // top of the heap
  kernel->outdev = consoleDDL; // the console defined in kernel32.c, output of
                               // kernel goes here
  ...
  memset(&kernel->regs, 0, sizeof(saveregs)); // initialize the execution
                                              // context
  kernel->regs.EIP = (DWORD)dex_init;      // dex_init() in kernel32.c
  kernel->regs.ESP = DISPATCHER_STACK_LOC; // set the values of the registers
                                           // for kernel mode process selector
  ...
  keyPCB.regs.EIP =
      (DWORD)kbdwrapper; // defined in irqwrap.asm to follow calling convention
