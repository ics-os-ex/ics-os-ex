DWORD time_count =
          0,       // used to store the number of seconds since dex was booted
    aux_time2 = 0; // since the OS has the timer set to interrupt 200 times a
                   // second an auxillary counter is required so that it
                   // increments time_count if it reaches 200

int time_monthdays[12] = {0,   31,  59,  90,  120, 151, 181,
...
char *datetostr(dex32_datetime *d, char *str) {
  ...
  sprintf(str, "%s/%s/%s", itoa(d->month, temp1, 10), itoa(d->day, temp2, 10),
          itoa(d->year, temp3, 10));
  return str;
};
...
void getdatetime(dex32_datetime *d) // gets the date nd time
{
  ...
  if (d->year < 80)
    d->year += 2000; // adjust for the year 2000
};

// returns time in milliseconds
DWORD time_gettime() { return time_count; };

// returns time in milliseconds
DWORD getprecisetime() { return (time_count * 100 + (aux_time2 / 2)); };
...
int time() {
  ...
  if (time_systime.month > 2 && time_systime.year % 4 == 0)
    totaldays += 1;
...
int time_getmycputime() { return current_process->totalcputime; };
