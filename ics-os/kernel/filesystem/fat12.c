...
int cached_fat_deviceid = -1;
DWORD cached_fat_size_bytes = 0;
static DWORD fat_cache_hits = 0;
static DWORD fat_cache_misses = 0;
static DWORD fat_read_clusters = 0;
static DWORD fat_last_report_ticks = 0;
static DWORD fat_last_report_clusters = 0;

DWORD fat_sectors_per_fat(BPB *bpbblock);

static void fat_perf_maybe_report() {
  DWORD now = getprecisetime();
  if (fat_last_report_ticks == 0) {
    fat_last_report_ticks = now;
    fat_last_report_clusters = fat_read_clusters;
    return;
  }
  if ((fat_read_clusters - fat_last_report_clusters) < 256 &&
      (now - fat_last_report_ticks) < 2000)
    return;
  if ((now - fat_last_report_ticks) >= 0) {
    serial_printf("FAT perf: cache_hit=%u cache_miss=%u read_clusters=%u\n",
                  fat_cache_hits, fat_cache_misses, fat_read_clusters);
    fat_last_report_ticks = now;
    fat_last_report_clusters = fat_read_clusters;
  }
}

static void ensure_fat_cached(BPB *bpbblock, int id) {
  DWORD fat_size_bytes = fat_sectors_per_fat(bpbblock) * 512;
  if (cached_fat_valid && cached_fat != 0 && cached_fat_deviceid == id &&
      cached_fat_size_bytes == fat_size_bytes) {
    fat_cache_hits++;
    fat_perf_maybe_report();
    return;
  }
  ...
  if (cached_fat != 0 &&
      (cached_fat_deviceid != id || cached_fat_size_bytes != fat_size_bytes)) {
    free(cached_fat);
    cached_fat = 0;
  }
...

  {
    ...
    DWORD handle = dex32_requestIO(id, IO_READ, start_of_fat,
                                   fat_sectors_per_fat(bpbblock), cached_fat);
  ...
  cached_fat_valid = 1;
  cached_fat_deviceid = id;
  cached_fat_size_bytes = fat_size_bytes;
  fat_perf_maybe_report();
}

int obtain_next_cluster(int cluster, void *fat, int fat_type, BPB *bpbblock,
                        int id) {
  WORD *temp;
  BYTE *fat12;
  WORD *fat16;
  ...
  if (fat_type == FAT12_FAT32) {
    if (fat == 0) {
      ensure_fat_cached(bpbblock, id);
      if (cached_fat_valid && cached_fat != 0) {
        fat32 = (DWORD *)cached_fat;
  ...
  if (fat_type == FAT12_FAT32) {
    if (fat == 0) {
      ...
      if (cached_fat_valid && cached_fat != 0) {
        ...
        DWORD fat_sector = (cluster >> 7) + start_of_fat;
        DWORD handle;
        fat32 = (DWORD *)(cached_fat + (fat_sector - start_of_fat) * 512);
      ...
      fat32 = (DWORD *)fat;
      fat32[cluster] = value & 0x0FFFFFFF;
      return cluster;
    }
...
int clustertoblock(BPB *bpbblock, int cluster) {
  ...
  int root_sectors =
      ((bpbblock->num_root_dir_ents * 32) + (bpbblock->bytes_per_sector - 1)) /
...
void interpretBPB(BPB *bpbblock) {
  printf("bytes per sector:%d\n sectors per cluster:%d\n bootsectors:%d\n",
         (WORD)bpbblock->bytes_per_sector, (BYTE)bpbblock->sectors_per_cluster,
         (WORD)bpbblock->num_boot_sectors);
...
void loadfat(BPB *bpbblock, void *fat, int id) {
  ...
  DWORD handle = dex32_requestIO(id, IO_READ, start_of_fat,
                                 fat_sectors_per_fat(bpbblock), fat);
  while (!dex32_IOcomplete(handle))
    ;
  dex32_closeIO(handle);
...
DWORD fat_readcluster(int cluster, BPB *bpbblock, char *buf, int id) {
  ...
  int i, i2, ind = 0, v = 0;
  handle = dex32_requestIO(id, IO_READ, start_sector,
                           bpbblock->sectors_per_cluster, buf);
  while (!dex32_IOcomplete(handle))
    ;
  dex32_closeIO(handle);
};
...
DWORD fat_writecluster(int cluster, BPB *bpbblock, char *buf, int id) {
  ...
  DWORD start_sector = clustertoblock(bpbblock, cluster);
  int i, i2, ind = 0, v = 0;
  handle = dex32_requestIO(id, IO_WRITE, start_sector,
                           bpbblock->sectors_per_cluster, buf);
  while (!dex32_IOcomplete(handle))
    ;
  dex32_closeIO(handle);
...
int fat_loaddirectoryfromcluster(DWORD start_clust, char **buf, int id) {
  ...
  if (fat_type != FAT12_FAT32) {
    fat = (BYTE *)malloc(fat_sectors_per_fat(&bpbblock) *
                         512); // allocate memory for FAT
...
// directory_size = total directory sectors x 512
  ...
  int directory_size =
      get_sector_fromcluster(cluster, &bpbblock, 0, fat, id) * 512;
  ...
  do {
    ...
    next_cluster = obtain_next_cluster(cluster, fat, fat_type, &bpbblock, id);

    if (next_cluster == cluster + 1 && ind < 999) {
      run_len++;
    } else {
      DWORD start_sector = clustertoblock(&bpbblock, run_start);
      DWORD blocks = run_len * bpbblock.sectors_per_cluster;
      DWORD handle = dex32_requestIO(id, IO_READ, start_sector, blocks,
                                     dirbuf + run_offset);
      while (!dex32_IOcomplete(handle))
        ;
      dex32_closeIO(handle);

      v += run_len * bpbblock.sectors_per_cluster * 512;
      ind += run_len;
      run_len = 0;
    }
...
int loadroot(fatdirentry **buf, const BPB *bpbblock, int id) {
  ...
  *buf = dir;
  // Load Them: We first queue the read request to the block device to the IO
  // manager
  handle = dex32_requestIO(id, IO_READ, blockloc, sector, (void *)dir);

  // Wait until the IO manager has finished the request and then close the
  // request handles
  while (!dex32_IOcomplete(handle))
    ;
...
int fat_getfreeblocks(int id) {
  ...
  readBPB(bpbblock, id);
  DWORD total = 0;
  fat = (BYTE *)malloc(bpbblock->sectors_per_fat *
                       512); // allocate memory for FAT
  loadfat(bpbblock, fat, id);
...
DWORD getdirsectorsize(fatdirentry *dir, BPB *bpbblock, int func, BYTE *fat,
                       int id) {
  return get_sector_fromcluster(dir->st_clust, bpbblock, func, fat, id);
};

DWORD get_sector_fromcluster(DWORD cluster, BPB *bpbblock, int func, BYTE *fat,
                             int id) {
  ...
  if (func == 1) // function 1: return last cluster
    ...
    return (
        ret *
        bpbblock->sectors_per_cluster); // function 2: return total *sectors*
};
...
int fat_getsectorsizeEX(vfs_node *f, int id) {
  ...
  fat =
      (BYTE *)malloc(fat_sectors_per_fat(buf) * 512); // allocate memory for FAT
  loadfat(buf, fat, id);
...
DWORD fat_createfile(vfs_node *f, BPB *bpbblock, int id) {
  ...
  vfs_node *parentdir =
      (vfs_node *)f->path; // the parent directory of the file ... : )
  BYTE *fat = 0;
  int ind = 0, fat_type;
  ...
  {
    dirsize =
        getdirsectorsize((fatdirentry *)parentdir->misc, bpbblock, 0, fat, id) *
        bpbblock->bytes_per_sector;
  ...
  for (i = 0; i < maxindex; i++) {
    if (dir[i].name[0] == 0xe5 || dir[i].name[0] == 0x00) {
      int spacemode = 0;
      dex32_datetime d;
      dos_date dosdate;
      ...
      // update the filename in the VFS to reflect the change to MS-DOS 8.3
      // format
      ...
      if (f->attb & FILE_DIRECTORY) {
        ...
        vfs_node *p;
        fc = obtainfreecluster(fat, bpbblock->total_sectors - 3);
        fat_write_cluster(fc, fat_get_eoc(fat_type), fat, fat_type, bpbblock,
                          id);
...
char *unicodetoascii(WORD *unicodestr, char *targ, int length) {
  ...
  for (i = 0; unicodestr[i] && ((length != 0 && i < length) || length == 0);
       i++)
...
int fat_mount(vfs_node *mountpoint, fatdirentry *buf2, BPB *bpb, int id) {
...

  {
    ...
    printf("fat: Adding %s\n", filename);
#endif
    if ((unsigned char)filename[0] == 0x00)
      break;
    ...
    if ((unsigned char)filename[0] != 0xe5 &&
      ...
      if ((buf2[i].attrib & ATTR_LONG_NAME_MASK) == ATTR_LONG_NAME) {
...
#ifdef FAT12_LFNSUPPORT
        if (strcmp(lfn_buffer, "") !=
            0) // A long filename was detected before this entry, so use it
        {
        ...
        if (buf2[i].ext[0] == 'E' && buf2[i].ext[1] == 'X' &&
            buf2[i].ext[2] == 'E')
          node->attb |= FILE_OEXE;
...
int fillsectorinfo(fatdirentry *dir, BPB *bpbblock, DWORD *sectinfo, int id) {
  unsigned int cluster = dir->st_clust, i, x, b, v = 0,
               n = 0; // obtain starting cluster
  WORD index, *temp;
  BYTE *fat;
#ifdef DEBUGX
  ...
  for (i = 0; i < fat_sectors_per_fat(bpbblock); i++) {
    DWORD handle = dex32_requestIO(id, IO_READ, bpbblock->num_boot_sectors + i,
                                   1, (void *)(fat + i * 512));
    while (!dex32_IOcomplete(handle))
      ;
    dex32_closeIO(handle);
...
int writefile12EX2(fatdirentry *dir, BPB *bpbblock, char *buf, int se,
  ...
  do {
    ...
    if (block >= startblock && block <= endblock) {
      ...
      if (block == startblock || block == endblock) {
        ...
        DWORD hdl = dex32_requestIO(id, IO_READ, sector, sectors_per_cluster,
                                    temp_buffer);
      ...
      handle = dex32_requestIO(id, IO_WRITE, sector, sectors_per_cluster,
                               temp_buffer);

      // since this is a write, which cannot be reordered, we must wait for it
      // to finish.
      while (!dex32_IOcomplete(handle))
        ;
      dex32_closeIO(handle);
...
int loadfile12EX2(fatdirentry *dir, BPB *bpbblock, char *buf, int se, int start,
                  int end, int id) {
  unsigned int cluster =
      ((DWORD)dir->st_clust_msw << 16) | (DWORD)dir->st_clust;
  ...
  cluster_buf = (BYTE *)malloc(cluster_requests * bytes_per_cluster);
  if (cluster_buf == 0)
    return 0;
  ...
  do {
    ...
    if (block >= startblock && block <= endblock) {
      if (next_cluster == cluster + 1 &&
          run_index + run_len < cluster_requests && block < endblock) {
        run_len++;
      } else {
        DWORD start_sector = clustertoblock(bpbblock, run_start);
        DWORD blocks = run_len * bpbblock->sectors_per_cluster;
        DWORD handle =
            dex32_requestIO(id, IO_READ, start_sector, blocks,
                            cluster_buf + (run_index * bytes_per_cluster));
        while (!dex32_IOcomplete(handle))
          ;
        dex32_closeIO(handle);
        fat_read_clusters += run_len;
        run_index += run_len;
        run_len = 0;
      }
    }
    ...
    cluster = next_cluster;
    block++;
  } while (cluster < fat_get_eoc(fat_type) && run_index < cluster_requests);

  // copy data read into the buffer
  ofs = 0;
  for (i = 0; i < (int)run_index; i++) {
    BYTE *src = cluster_buf + (i * bytes_per_cluster);
    if (i == startblock && i == endblock) {
      memcpy((void *)buf, src + startadj, end - start + 1);
      ofs += (end - start);
      break;
    } else if (i == startblock) {
      memcpy((void *)buf, src + startadj, startlength);
      ofs += startlength;
    } else if (i == endblock) {
      memcpy((void *)(buf + ofs), src, adj);
      ofs += adj;
      break;
    } else {
      memcpy((void *)(buf + ofs), src, bytes_per_cluster);
      ofs += bytes_per_cluster;
    };
  };
