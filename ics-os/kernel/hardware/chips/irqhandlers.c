/*
  ...
  Date: 13/03/04 06:30
  Description: This module handles registration of interrupt handlers and also
  manages the Programmable interrupt controller.
...
void unhandled(int irq_num) {
  ...
  //  while (1) {}; //temporarily disabled infinite loop for unhandled
  //  interrupts
  startints();
};
...
parameters:
    ...
    handler    = a pointer to the event handler of the IRQ which is supplied by
the device driver. returns: -1 if unsuccessful 0 if handler was the first 1 if
handler was not the first to be hooked onto the irq
====================================================================================
*/
...
/*
====================================================================================
This procedure "setdefaulthandlers()" sets up the IDT (Interrupt Descriptor
Table)
--The Interrupt Descriptor Table (IDT) is like the IVT in x86 real mode only
that an entry in an IDT contains more elements like security bits, and uses
...
void setdefaulthandlers() {
  ...
  setinterruptvector(0x21, dex_idtbase, 0x8E, irq1wrapper, SYS_CODE_SEL);

  setinterruptvector(0x22, dex_idtbase, 0x8E, irq2wrapper, SYS_CODE_SEL);

  setinterruptvector(0x23, dex_idtbase, 0x8E, irq3wrapper, SYS_CODE_SEL);

  setinterruptvector(0x24, dex_idtbase, 0x8E, irq4wrapper, SYS_CODE_SEL);

  setinterruptvector(0x25, dex_idtbase, 0x8E, irq5wrapper, SYS_CODE_SEL);

  setinterruptvector(0x26, dex_idtbase, 0x8E, irq6wrapper, SYS_CODE_SEL);

  setinterruptvector(0x27, dex_idtbase, 0x8E, irq7wrapper, SYS_CODE_SEL);

  setinterruptvector(0x28, dex_idtbase, 0x8E, irq8wrapper, SYS_CODE_SEL);

  setinterruptvector(0x29, dex_idtbase, 0x8E, irq9wrapper, SYS_CODE_SEL);

  setinterruptvector(0x2A, dex_idtbase, 0x8E, irq10wrapper, SYS_CODE_SEL);

  setinterruptvector(0x2B, dex_idtbase, 0x8E, irq11wrapper, SYS_CODE_SEL);

  /************************** Install the CPU exception handlers
   * *****************/
  // install the GPF handler
  setinterruptvector(0x0D, dex_idtbase, 0x8E, gpfwrapper, SYS_CODE_SEL);

  // install the page fault handler
  setinterruptvector(14, dex_idtbase, 0x85, 0, PF_TSS);

  // install some error handlers
  setinterruptvector(0x0, dex_idtbase, 0x8E, div_wrapper, SYS_CODE_SEL);

  setinterruptvector(11, dex_idtbase, 0x8E, seg_error, SYS_CODE_SEL);

  setinterruptvector(12, dex_idtbase, 0x8E, stack_error, SYS_CODE_SEL);

  setinterruptvector(10, dex_idtbase, 0x8E, invalidtsswrapper, SYS_CODE_SEL);

  setinterruptvector(4, dex_idtbase, 0x8E, overflow, SYS_CODE_SEL);

  setinterruptvector(9, dex_idtbase, 0x8E, coprocessor_segment_overrun,
                     SYS_CODE_SEL);

  setinterruptvector(16, dex_idtbase, 0x8E, copwrapper, SYS_CODE_SEL);

  setinterruptvector(7, dex_idtbase, 0x8E, copwrapper, SYS_CODE_SEL);

  setinterruptvector(5, dex_idtbase, 0x8E, boundscheck, SYS_CODE_SEL);

  setinterruptvector(8, dex_idtbase, 0x8E, double_fault, SYS_CODE_SEL);

  setinterruptvector(6, dex_idtbase, 0x8E, fdcwrapper, SYS_CODE_SEL);

  setinterruptvector(3, dex_idtbase, 0x8E, breakpoint, SYS_CODE_SEL);

  setinterruptvector(1, dex_idtbase, 0x8E, debug_error, SYS_CODE_SEL);

  setinterruptvector(0x30, dex_idtbase, 0xEE, syscallwrapper, SYS_CODE_SEL);

  setinterruptvector(0x31, dex_idtbase, 0x8E, syscallwrapper, SYS_CODE_SEL);

  intloc.limit = 2047;
  intloc.location = dex_idtbase;
  loadregisters(); // load the idtr register with data, defined in
                   // kernel/startup/asmlib.asm
};
