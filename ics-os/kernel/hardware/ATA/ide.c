/*
  ...
  Date: 02/02/04 04:10
  Description: The ATA/IDE driver for harddisks and CD-ROM drives, also includes
  a partition detecteor. There is no capability for detecting extended
  partitions yet though.
...
// Per-interface reg_config_info storage (primary=0, secondary=1)
// This is needed because the low-level ATA driver uses global reg_config_info
// which gets overwritten when probing the second IDE controller.
static int ide_reg_config_info[2][2];  // [interface][device]
...
unsigned char *devTypeStr[] = {"no device found", "unknown type", "ATA",
                               "ATAPI"};
...
void ide_irqhandler() { ide_ready = 1; };

void ide_getdriveinfo() {

};

int ide_readsectors(int interface, int dev, DWORD lba, DWORD sectors,
                    char *buffer) {
  int base;
  int i, ofs = 0;
  ...
  // Restore reg_config_info for this interface
  reg_config_info[0] = ide_reg_config_info[interface][0];
  reg_config_info[1] = ide_reg_config_info[interface][1];
  ...
  do {
    ...
    reg_pio_data_in_lba(dev, CMD_READ_SECTORS, 0, total_sectors,
                        lba + block_ofs, SYS_DATA_SEL, buffer + ofs,
                        total_sectors, 0);
...
int ide_writesectors(int interface, int dev, DWORD lba, DWORD sectors,
                     char *buffer) {
  int base;
  int i, ofs = 0;
  int total_sectors = 0, block_ofs = 0;
...
int ide_cdromreadsectors(int interface, int dev, DWORD lba, DWORD sectors,
                         char *buffer) {
  ...
  stat = reg_packet(dev, 12, SYS_DATA_SEL, &atapi_seek_packet, 0, 0,
                    SYS_DATA_SEL, 0);
  ...
  // ATAPI lba address format and blocks seems to be in BIG endian... so we have
  ...
  if (reg_packet(dev, 12, SYS_DATA_SEL, &atapi_packet, 0, 2048, SYS_DATA_SEL,
                 buffer) != 0) {
    printf("read failure!\n");
    return -1;
  };
...
int ide_cdromgetblocksize() { return 2048; };

// A universal get bytes per block function for all ATA devices
int ide_uni_get_bytes_per_block() {
  int info_index = -1;
  int i;
  /* get the index of the drive information structure associated with this
     device depending on which device DEX used to call this function, this is
     done using the context inforation set by the bridge when interfacing with
     this module*/
  for (i = 0; i < 4; i++) {
...
int ide_uni_get_total_sectors() {
  ...
  return ide_drivelist[info_index].sectors_per_track *
         ide_drivelist[info_index].cylinder * ide_drivelist[info_index].heads;
};
...
int ide_uni_read_block(int block, char *blockbuff, int numblocks) {
  ...
  if (ide_drivelist[info_index].asus_adjustment)
    block -= 1;
  ...
  if (!ide_drivelist[info_index].atapi)
    return ide_readsectors(ide_drivelist[info_index].interface,
                           ide_drivelist[info_index].dev, block, numblocks,
                           blockbuff);
  else
...
int ide_read_block_partition(int block, char *blockbuff, int numblocks) {
  ...
  for (i = 0; i < ide_total_partition; i++) {
    ...
    if (ide_partitions[i].mydeviceid == device_context) {
      ...
      myblock =
          (devmgr_block_desc *)devmgr_getdevice(ide_partitions[i].hd_deviceid);
      ...
      return bridges_call(myblock, &myblock->read_block,
                          block + ide_partitions[i].startlba, blockbuff,
                          numblocks);
    };
  };
...
const char *ide_identify_partition_type(int part_type) {
  ...
  case 7:
    ...
    break;
  case 0xb:
    return "Windows95 FAT32";
    break;
  case 0xc:
    return "Windows95 FAT32 (LBA)";
    break;
  case 0xE:
    return "Windows95 FAT16 (LBA)";
    break;
  case 0xF:
    return "Windows95 Extended (LBA)";
    break;
  case 0x82:
    return "Linux Swap";
    break;
  case 0x83:
    return "Linux native fs or ext2fs";
    break;
  case 0x85:
...
int ide_registerpartitions(int deviceid) {
  ...
  // obtain partition information from the MBR(Master Boot Record) of the target
  // HDD
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (mbr->tables[i].type != 0) {
      ...
      sprintf(mypartition.hdr.description, "%s on partition %d at %s",
              ide_identify_partition_type(mbr->tables[i].type), i,
              myblock->hdr.name);
      ...
      ide_partitions[pindex].endlba =
          mbr->tables[i].startlba + mbr->tables[i].sector_size;
    };
  };
  ...
  if (type == DEVMGR_MESSAGESTR) {
    ...
    if (p != 0) {
      if ((strcmp(p, "-eject") == 0) && ide_drivelist[info].atapi) {
        startints();
        ide_cdrom_eject(ide_drivelist[info].interface, ide_drivelist[info].dev);

      } else if ((strcmp(p, "-load") == 0) && ide_drivelist[info].atapi) {
        startints();
        ide_cdrom_load(ide_drivelist[info].interface, ide_drivelist[info].dev);

      } else if ((strcmp(p, "-lock") == 0) && ide_drivelist[info].atapi) {
        startints();
        ide_cdrom_lock(ide_drivelist[info].interface, ide_drivelist[info].dev);

      } else if ((strcmp(p, "-unlock") == 0) && ide_drivelist[info].atapi) {
        startints();
        ide_cdrom_unlock(ide_drivelist[info].interface,
                         ide_drivelist[info].dev);

      } else if (strcmp(p, "-asus") == 0) {
      ...
      if (ide_drivelist[info].atapi) {
        ...
        printf(
            "-write_enable : Write enables the IDE drive (!!Experimental!!)\n");
...
void ide_setupinterface(int type, int intnum, const char *interface_str) {
  ...
  // Save reg_config_info for this interface
  ide_reg_config_info[type][0] = reg_config_info[0];
  ide_reg_config_info[type][1] = reg_config_info[1];
  ...
  if (reg_config_info[1] == 2)
    hd2 = 1;
  if (reg_config_info[0] == 3)
    cd1 = 1;
  if (reg_config_info[1] == 3)
    cd2 = 1;
  ...
  if (hd1) {
    ...
    reg_pio_data_in(0, CMD_IDENTIFY_DEVICE, 0, 0, 0, 0, 0, SYS_DATA_SEL, buffer,
                    1L, 0);

    // make it visible to the device manager
    memset(&myblock, 0, sizeof(myblock));
    sprintf(myblock.hdr.name, "hd%s0", interface_str);
    sprintf(myblock.hdr.description, "ATA %s master HDD (READ-ONLY)",
            interface_str);
  ...
  if (hd2) {
    reg_pio_data_in(1, CMD_IDENTIFY_DEVICE, 0, 0, 0, 0, 0, SYS_DATA_SEL, buffer,
                    1L, 0);

    // make it visible to the device manager
    memset(&myblock, 0, sizeof(myblock));
    sprintf(myblock.hdr.name, "hd%s1", interface_str);
    sprintf(myblock.hdr.description, "ATA %s Slave HDD (READ-ONLY)",
            interface_str);
    myblock.hdr.type = DEVMGR_BLOCK;
    myblock.hdr.size = sizeof(myblock);
  ...
  if (cd1) {
    reg_pio_data_in_lba(0, CMD_IDENTIFY_DEVICE_PACKET, 0, 0, 0L, SYS_DATA_SEL,
                        buffer, 1L, 0);

    // make it visible to the device manager
    memset(&myblock, 0, sizeof(myblock));
    sprintf(myblock.hdr.name, "cd%s0", interface_str);
    sprintf(myblock.hdr.description, "ATA %s master CD/DVD-ROM (READ-ONLY)",
            interface_str);
    myblock.hdr.type = DEVMGR_BLOCK;
    myblock.hdr.size = sizeof(myblock);
    ...
    ide_drivelist[index].mydeviceid =
  ...
  if (cd2) {
    reg_pio_data_in_lba(1, CMD_IDENTIFY_DEVICE_PACKET, 0, 0, 0L, SYS_DATA_SEL,
                        buffer, 1L, 0);

    // make it visible to the device manager
    memset(&myblock, 0, sizeof(myblock));
    sprintf(myblock.hdr.name, "cd%s1", interface_str);
    sprintf(myblock.hdr.description, "ATA %s slave CD/DVD-ROM (READ-ONLY)",
            interface_str);
    myblock.hdr.type = DEVMGR_BLOCK;
    myblock.hdr.size = sizeof(myblock);
    ...
    ide_drivelist[1 + index].mydeviceid =
        devmgr_register((devmgr_generic *)&myblock);
