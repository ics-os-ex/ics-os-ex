...
int reg_non_data_lba(int dev, int cmd, int fr, int sc, long lba)

{
  ...
  return reg_non_data(dev, cmd, fr, sc, cyl, head, sect);
}
...
int reg_non_data(int dev, int cmd, int fr, int sc, unsigned int cyl, int head,
                 int sect)

{
  ...
  if (cmd != CMD_DEVICE_RESET) {
    ...
    printf("reg_pio_data_in: sub_select done\n");
    ...
    printf("reg_pio_data_in: sending cmd %x\n", cmd);
...
int reg_pio_data_in_lba(int dev, int cmd, int fr, int sc, long lba,
                        unsigned seg, unsigned off, int numSect, int multiCnt)

{
  ...
  lba = lba >> 16;
  head = ((int)(lba & 0x0fL)) | 0x40;
  return reg_pio_data_in(dev, cmd, fr, sc, cyl, head, sect, seg, off, numSect,
                         multiCnt);
}
...
int reg_pio_data_in(int dev, int cmd, int fr, int sc, unsigned int cyl,
                    int head, int sect, unsigned seg, unsigned off, int numSect,
                    int multiCnt)

{
  ...
  printf("reg_pio_data_in: loop start\n");
...
int reg_pio_data_out_lba(int dev, int cmd, int fr, int sc, long lba,
                         unsigned seg, unsigned off, int numSect, int multiCnt)

{
  ...
  lba = lba >> 16;
  head = ((int)(lba & 0x0fL)) | 0x40;
  return reg_pio_data_out(dev, cmd, fr, sc, cyl, head, sect, seg, off, numSect,
                          multiCnt);
}
...
int reg_pio_data_out(int dev, int cmd, int fr, int sc, unsigned int cyl,
                     int head, int sect, unsigned seg, unsigned off,
...
int reg_packet(int dev, unsigned int cpbc, unsigned int cpseg,
               unsigned int cpoff, int dir, long dpbc, unsigned int dpseg,
               unsigned int dpoff)

{
  ...
  if ((reason & (CB_SC_P_TAG | CB_SC_P_REL)) || (!(reason & CB_SC_P_IO)) ||
      (!(reason & CB_SC_P_CD)))
                            ...
                            long numSect, int multiCnt )
{
   ...
   while ( 1 )
   {
      ...
      sub_atapi_delay( dev );
      printf("reg_pio_data_in: waiting for interrupt\n");
      reg_wait_poll( 34, 35 );
      printf("reg_pio_data_in: got interrupt\n");
