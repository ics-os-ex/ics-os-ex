...
| `test-net` | QEMU virtio-net + SLIRP; DHCP lease + ICMP + UDP `:7777` + TCP `:7778` + userland sockets (`netecho.exe` TCP/UDP/listen); `NET_DHCP_OK`/`NET_PING_OK`/`NET_UDP_OK`/`NET_TCP_OK`/`NET_SOCK_OK`/`NET_SOCK_UDP_OK`/`NET_SOCK_LISTEN_OK` |
